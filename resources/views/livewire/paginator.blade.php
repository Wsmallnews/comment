<div class="container mx-auto">
    @if ($pageType == 'scroll')
        @if ($pageInfo['load_status'] == 'loading')
            <div class="flex justify-center items-center text-gray-400" x-intersect="$wire.$parent.nextPage('{{ $pageName }}')">
        ...
        <div class="relative text-sm text-gray-400 flex items-center">
            ...
            @if ($pageInfo['load_status'] == 'loading')
                ...
                <div wire:loading.remove>
                    <span class="cursor-pointer" wire:click="$parent.nextPage('{{ $pageName }}')">展开更多</span>
                </div>
            @elseif ($pageInfo['load_status'] == 'empty')
                <span>暂没有评论数据</span>
            @elseif ($pageInfo['load_status'] == 'nomore')

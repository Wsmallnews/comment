...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('sn_comments', function (Blueprint $table) {
            ...
            $table->string('commenter_avatar')->nullable()->comment('评论者头像');

            $table->morphs('be_replyer');
            $table->string('be_replyer_name')->nullable()->comment('被回复者昵称');
            $table->string('be_replyer_avatar')->nullable()->comment('被回复者头像');

            $table->string('content', 2048)->nullable()->comment('评论内容');
            $table->json('images')->nullable()->comment('评论图片');

            $table->unsignedInteger('likes')->default(0)->comment('喜欢数量');
            $table->unsignedInteger('comments')->default(0)->comment('评论数量');
